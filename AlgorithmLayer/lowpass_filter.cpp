...
/**
  ...
  */
int LowpassFilter::init(LPF_Handle& filter, float Tf)
{
    filter.Tf = Tf;
...
/**
  ...
  * @retval         滤波后的输出值
  */
int LowpassFilter::update(LPF_Handle& filter, float input, float& output)
{
    // 获取当前时间戳
...
extern "C" int LPF_Init(LPF_Handle* filter, float Tf)
{
    if (filter == NULL) {
        log_error("Parameter error!");
        return E_PARAM;
    }
    return LowpassFilter::init(*filter, Tf);
}

extern "C" int LPF_Update(LPF_Handle* filter, float input, float* output)
{
    if (filter == NULL || output == NULL) {
        log_error("Parameter error!");
        return E_PARAM;
    }
    return LowpassFilter::update(*filter, input, *output);
}
