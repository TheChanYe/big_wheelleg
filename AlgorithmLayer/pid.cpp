...
/*PID参数初始化*/
int PidController::init(PID_Handle& pid, const PID_Params& params)
{
    pid.kp = params.kp;
    pid.ki = params.ki;
    pid.kd = params.kd;
...
/* PID计算函数（含抗积分饱和） */
int PidController::calculate(PID_Handle& pid, float target, float current,
                             float& output)
{
    // 计算误差
...
/* PID复位 — 清零所有历史状态，输出归零 */
int PidController::reset(PID_Handle& pid)
{
    pid.error      = 0.0f;
    pid.integral   = 0.0f;
    pid.prev_error = 0.0f;
...
extern "C" int PID_Init(PID_Handle* pid, PID_Params* pid_params)
{
    if (pid == NULL || pid_params == NULL) {
        log_error("Parameter error!");
        return E_PARAM;
    }
    return PidController::init(*pid, *pid_params);
}

extern "C" int PID_Calc(PID_Handle* pid, float target, float current,
                         float* output)
{
    ...
    return PidController::calculate(*pid, target, current, *output);
}

extern "C" int PID_Reset(PID_Handle* pid)
{
    if (pid == NULL) {
        log_error("PID_Reset: null pointer");
        return E_PARAM;
    }
    return PidController::reset(*pid);
}
