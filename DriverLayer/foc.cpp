...
static int Motor_Calib(Motor_Data* motor)
{
	...
	if(ret == E_OK)
	{
		...
		while(motor->calib.Angle_Cailb_State == WAIT)//等待角度校准完成
		{
			...
			vTaskDelay(300);
			/* 获取互斥锁（带超时） */
			if(xSemaphoreTake(System_GetMutex(), portMAX_DELAY) != pdTRUE)
			{
						log_error("SemaphoreTake Wait timeout!");
				xSemaphoreGive(System_GetMutex()); // 先释放锁！
						return E_ERROR;
			}
			if(motor->tmr == TMR1)
			{	
			...
			xSemaphoreGive(System_GetMutex());//释放互斥锁
...
int Motor_CheckEncoder(Motor_Data *motor)
{
    ...
    if (motor == NULL || xSemaphoreTake(System_GetMutex(), pdMS_TO_TICKS(10)) != pdTRUE)
        return E_ERROR;
    if (motor->tmr == TMR1)
